...
`main` is protected against deletion and force-pushes, but not behind required review: the release workflow pushes to it directly, and on a personal repository GitHub Actions cannot be granted a ruleset bypass — that is an organisation-level feature. Requiring pull requests would therefore mean giving the workflow a deploy key or token of its own, trading a review gate for a long-lived credential in the repository's secrets. Pull-request CI runs on `pull_request` rather than `pull_request_target`, so a fork's code never runs with a write token or access to secrets, and every external contributor's run needs approval before it starts.
